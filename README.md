...
Site de ebooks para hospedagem no Render. Funciona como **front/máscara** do checkout (PayPal + **Paddle Billing**) — quando o provedor verifica a origem, vê este domínio de ebooks.
...
5. **Environment Variables** no Render:
   ...
   - `PAYPAL_CLIENT_ID` — Checkout PayPal (`method=paypal`)
...
O fluxo continua: o cliente sai do catálogo de vídeos e abre o checkout **neste** domínio (ebooks), com `method=paypal` ou `method=paddle`.
